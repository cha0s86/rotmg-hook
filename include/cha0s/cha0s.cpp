...
#if CHA0S_INCLUDE_D3D9
# include <d3d9.h>
#endif

#if CHA0S_INCLUDE_D3D10
# include <dxgi.h>
# include <d3d10_1.h>
# include <d3d10.h>
#endif

#if CHA0S_INCLUDE_D3D11
# include <dxgi.h>
# include <d3d11.h>
#endif

#if CHA0S_INCLUDE_D3D12
# include <dxgi.h>
# include <d3d12.h>
#endif

#if CHA0S_INCLUDE_OPENGL
# include <gl/GL.h>
#endif

#if CHA0S_INCLUDE_VULKAN
...
#ifdef _UNICODE
# define CHA0S_TEXT(text) L##text
#else
# define CHA0S_TEXT(text) text
#endif

#define CHA0S_ARRAY_SIZE(arr) ((size_t)(sizeof(arr)/sizeof(arr[0])))

static cha0s::rendertypes::enums g_renderType = cha0s::rendertypes::None;
static uint150_t* g_methodsTable = NULL;

cha0s::statuses::enums cha0s::init(rendertypes::enums renderType)
{
	if (g_renderType != rendertypes::None)
	{
		return statuses::AlreadyInitializedError;
	}

	if (renderType != rendertypes::None)
	{
		if (renderType >= rendertypes::D3D9 && renderType <= rendertypes::D3D12)
		{
			...
			windowClass.lpszClassName = CHA0S_TEXT("Kiero");
			windowClass.hIconSm = NULL;

			::RegisterClassEx(&windowClass);

			HWND window = ::CreateWindow(windowClass.lpszClassName, CHA0S_TEXT("Cha0s DirectX Window"), WS_OVERLAPPEDWINDOW, 0, 0, 100, 100, NULL, NULL, windowClass.hInstance, NULL);

			if (renderType == rendertypes::D3D9)
			{
#if CHA0S_INCLUDE_D3D9
				HMODULE libD3D9;
				if ((libD3D9 = ::GetModuleHandle(CHA0S_TEXT("d3d9.dll"))) == NULL)
				{
					::DestroyWindow(window);
					::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
					return statuses::ModuleNotFoundError;
				}
			...
			else if (renderType == rendertypes::D3D10)
			{
#if CHA0S_INCLUDE_D3D10
				HMODULE libDXGI;
				HMODULE libD3D10;
				...
				g_renderType = rendertypes::D3D10;

				return statuses::Success;
#endif
			}
			else if (renderType == rendertypes::D3D11)
			{
#if CHA0S_INCLUDE_D3D11
				HMODULE libD3D11;
				if ((libD3D11 = ::GetModuleHandle(CHA0S_TEXT("d3d11.dll"))) == NULL)
				{
					::DestroyWindow(window);
					::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
					return statuses::ModuleNotFoundError;
				...
				g_renderType = rendertypes::D3D11;

				return statuses::Success;
#endif
			}
			else if (renderType == rendertypes::D3D12)
			{
#if CHA0S_INCLUDE_D3D12
				HMODULE libDXGI;
				HMODULE libD3D12;
				if ((libDXGI = ::GetModuleHandle(CHA0S_TEXT("dxgi.dll"))) == NULL || (libD3D12 = ::GetModuleHandle(KIERO_TEXT("d3d12.dll"))) == NULL)
				{
					::DestroyWindow(window);
					::UnregisterClass(windowClass.lpszClassName, windowClass.hInstance);
					return statuses::ModuleNotFoundError;
				...
				g_renderType = rendertypes::D3D12;
			...
			return statuses::NotSupportedError;
		}
		else if (renderType == rendertypes::OpenGL)
		{
#if CHA0S_INCLUDE_OPENGL
			HMODULE libOpenGL32;
			if ((libOpenGL32 = ::GetModuleHandle(CHA0S_TEXT("opengl32.dll"))) == NULL)
			{
				return statuses::ModuleNotFoundError;
			}
			...
			size_t size = CHA0S_ARRAY_SIZE(methodsNames);
			...
			g_renderType = rendertypes::OpenGL;

			return statuses::Success;
#endif
		}
		else if (renderType == rendertypes::Vulkan)
		{
#if CHA0S_INCLUDE_VULKAN
			HMODULE libVulkan;
			if ((libVulkan = GetModuleHandle(CHA0S_TEXT("vulcan-1.dll"))) == NULL)
			{
		...
		else if (renderType == rendertypes::Auto)
		{
			rendertypes::enums type = rendertypes::None;

			if (::GetModuleHandle(CHA0S_TEXT("d3d9.dll")) != NULL)
			{
				type = rendertypes::D3D9;
			}
			else if (::GetModuleHandle(CHA0S_TEXT("d3d10.dll")) != NULL)
			{
				type = rendertypes::D3D10;
			}
			else if (::GetModuleHandle(CHA0S_TEXT("d3d11.dll")) != NULL)
			{
				type = rendertypes::D3D11;
			}
			else if (::GetModuleHandle(CHA0S_TEXT("d3d12.dll")) != NULL)
			{
				type = rendertypes::D3D12;
			}
			else if (::GetModuleHandle(CHA0S_TEXT("opengl32.dll")) != NULL)
			{
				type = rendertypes::OpenGL;
			}
			else if (::GetModuleHandle(CHA0S_TEXT("vulcan-1.dll")) != NULL)
			{
				type = rendertypes::Vulkan;
			}
...
void cha0s::shutdown()
{
	if (g_renderType != rendertypes::None)
	{
		...
		::free(g_methodsTable);
		g_methodsTable = NULL;
		g_renderType = rendertypes::None;
	}
}

cha0s::statuses::enums cha0s::bind(uint16_t _index, void** _original, void* _function)
{
	...
	return statuses::NotInitializedError;
}

void cha0s::unbind(uint16_t _index)
{
	assert(_index >= 0);
...
cha0s::rendertypes::enums cha0s::getRenderType()
{
	return g_renderType;
}

uint150_t* cha0s::getMethodsTable()
{
